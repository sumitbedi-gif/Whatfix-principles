...
function FrameworkLayout({ principle }: { principle: Principle }) {
  ...
  const [activeKey, setActiveKey] = useState<PactKey>(pact[0]?.key ?? 'people')
  const activeDimension = pact.find((d) => d.key === activeKey)
  ...
  return (
    <div className="flex h-screen flex-col lg:flex-row">
      ...
      {/* RIGHT — persona gallery, driven by the active PACT lens */}
      <div className="relative flex flex-1 flex-col bg-white p-5 lg:py-7 lg:pr-7">
        <div className="flex-1 rounded-3xl bg-panel p-6 sm:p-8">
          {principle.personas && activeDimension && (
            <PersonaGallery
